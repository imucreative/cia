<?php
	class Mod_product extends CI_Model{
		...
		public $table_prod_tags	= "tbl_product_tags";
		...
		function simpan($gambar){
			...
			if(count($tag_list) > 0){
				for($i=0; $i<count($tag_list); $i++){
					...
					$this->db->insert($this->table_prod_tags, $data_tags);
				}
			}
		...
		//produk tag
		function simpan_tag_produk($product_id){
			$data = array(
				'id_tags'		=> $this->input->post('tags'),
				'product_id'	=> $product_id
			);
			$this->db->insert($this->table_prod_tags, $data);
		}
		
		function hapus_tag_produk($id){
			$data = array(
				'status_delete'    => "1"
			);
			$this->db->where('product_tags_id', $id);
			$this->db->update($this->table_prod_tags, $data);
		}
...
//=======================================================================================================================================
		...
		function get_product_by_kategori_id($kategori_id){
			$this->db->where('status_delete', 0);
			$this->db->where('publish', 1);
			$this->db->where('kategori_id', $kategori_id);
			$query		= $this->db->get($this->table_produk);
...
//=======================================================================================================================================
		...
		//produk tag
		function get_product_tags_by_product_id($product_id){
			$this->db->where('status_delete', 0);
			$this->db->where('product_id', $product_id);
			$query		= $this->db->get($this->table_prod_tags);
			return $query;
		}
